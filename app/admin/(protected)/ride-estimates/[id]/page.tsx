...
export default async function EditRideEstimatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: estimate }, { data: rawCompanies }, { data: cities }, { data: sources }] = await Promise.all([
    supabaseAdmin.from("ride_estimates").select("*").eq("id", id).single(),
    ...
    <div className="max-w-lg">
      ...
      <form action={update} className="flex flex-col gap-4 bg-white border border-gray-200 rounded-lg p-6 mb-6">
        <div>
          ...
          <select name="company_id" required defaultValue={estimate.company_id} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
        ...
        <div>
          ...
          <select name="city_id" defaultValue={estimate.city_id ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input name="period_start" type="date" required defaultValue={estimate.period_start} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Period end *</label>
            <input name="period_end" type="date" required defaultValue={estimate.period_end} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Rides per week *</label>
          <input name="rides_per_week" type="number" required min="0" defaultValue={estimate.rides_per_week} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Avg fare (USD)</label>
          <input name="avg_fare_usd" type="number" step="0.01" min="0" defaultValue={estimate.avg_fare_usd ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Confidence *</label>
          <select name="confidence" required defaultValue={estimate.confidence} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
        ...
        <div>
          ...
          <select name="source_id" defaultValue={estimate.source_id ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
        ...
        <div>
          ...
          <textarea name="methodology_note" rows={3} defaultValue={estimate.methodology_note ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
