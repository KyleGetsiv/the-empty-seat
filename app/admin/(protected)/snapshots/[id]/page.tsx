...
export default async function EditSnapshotPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: row }, { data: programs }, { data: sources }] = await Promise.all([
    supabaseAdmin.from("competitor_snapshots").select("*").eq("id", id).single(),
  ...
  async function update(formData: FormData) {
    "use server";
    const { error } = await supabaseAdmin.from("competitor_snapshots").update({
    ...
    if (error) {
      console.error("[update competitor_snapshots]", error);
      throw new Error(`Failed to update competitor_snapshots row: ${error.message}`);
    }
  ...
  async function remove() {
    "use server";
    const { error } = await supabaseAdmin.from("competitor_snapshots").delete().eq("id", id);
    if (error) {
      console.error("[delete competitor_snapshots]", error);
      throw new Error(`Failed to delete competitor_snapshots row: ${error.message}`);
    }
    revalidatePath("/landscape");
    redirect("/admin/snapshots");
  }
    ...
    <div className="max-w-lg">
      ...
      <form action={update} className="flex flex-col gap-4 bg-white border border-gray-200 rounded-lg p-6 mb-6">
        <div>
          ...
          <select name="program_id" required defaultValue={row.program_id} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
        ...
        <div>
          ...
          <input name="snapshot_date" type="date" required defaultValue={row.snapshot_date} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Cities serving public</label>
          <input name="cities_serving_public" type="number" step="any" defaultValue={row.cities_serving_public ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Cities operating (incl. employee/supervised)</label>
          <input name="cities_operating_total" type="number" step="any" defaultValue={row.cities_operating_total ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Vehicle count</label>
          <input name="vehicle_count" type="number" step="any" defaultValue={row.vehicle_count ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Weekly rides</label>
          <input name="weekly_rides" type="number" step="any" defaultValue={row.weekly_rides ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Cumulative rides</label>
          <input name="cumulative_rides" type="number" step="any" defaultValue={row.cumulative_rides ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Autonomous miles (cumulative)</label>
          <input name="autonomous_miles_cumulative" type="number" step="any" defaultValue={row.autonomous_miles_cumulative ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Funding total (USD)</label>
          <input name="funding_total_usd" type="number" step="any" defaultValue={row.funding_total_usd ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Implied valuation (USD)</label>
          <input name="implied_valuation_usd" type="number" step="any" defaultValue={row.implied_valuation_usd ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Supervision</label>
            <select name="supervision" defaultValue={row.supervision ?? ""} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
          ...
          <div>
            ...
            <select name="disclosure_quality" required defaultValue={row.disclosure_quality} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none">
