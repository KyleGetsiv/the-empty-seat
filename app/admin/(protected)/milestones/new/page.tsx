...
export default async function NewMilestonePage() {
  ...
  const { data: sources } = await supabaseAdmin.from("sources").select("id, title, publisher").order("created_at", { ascending: false }).limit(100);
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-semibold text-gray-900">New milestone</h1>
      </div>
      ...
      <form action={create} className="flex flex-col gap-4 bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="flex items-center justify-between rounded-lg bg-yellow-50 border border-yellow-200 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-gray-500">Toggle to make visible on the public site</p>
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            <input name="is_published" type="checkbox" className="sr-only peer" />
        ...
        <div>
          ...
          <input name="headline" required className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Body (markdown)</label>
          <textarea name="body" rows={6} className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none font-mono" />
        </div>
        <div>
        ...
        <div>
          ...
          <input name="kyle_annotation" className="w-full rounded border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" />
        </div>
